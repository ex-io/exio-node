...
class Client {
  ...
  constructor(config) {
    this.config = { ...config };
    this._restApi = this.config.restApi || `https://api.${this.config.domain}`;
    this._websocketApi =
      this.config.websocketApi || `wss://feed.${this.config.domain}`;
  }
   ...
   * @param {mixed} channels - String for channel to subscribe to, or array of channels
   * @param {mixed} symbols - String for symbol to subscribe to, or array of symbols
  ...
  subscribe(channels, symbols) {
    ...
    for (let i = 0, sz = channels.length; i < sz; ++i) {
      msg.channels.push({
        name: channels[i],
        symbols: symbols
      });
